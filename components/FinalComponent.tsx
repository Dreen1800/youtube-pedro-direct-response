...
export default function FinalComponent({ balance }: FinalComponentProps) {
  ...
  useEffect(() => {
    ...
    // Carregamento do script do player ConvertEai específico
    const loadPlayerScript = () => {
      try {
        const existingScript = document.querySelector('script[src*="68cf097572c453bf1773b5ab"]')
        if (!existingScript) {
          const s = document.createElement("script")
          s.type = "text/javascript"
          s.src = "https://scripts.converteai.net/ff9f6de5-a5a0-4221-9188-aae68066cbeb/players/68cf097572c453bf1773b5ab/v4/player.js"
          s.async = true
          s.onload = () => console.log("ConvertEai Black Content player script loaded successfully")
          s.onerror = () => console.error("Failed to load ConvertEai Black Content player script")
          document.head.appendChild(s)
        }
      } catch (error) {
        console.error("Error loading ConvertEai Black Content player script:", error)
      }
    }
            ...
            {isClient ? (
              <div 
                dangerouslySetInnerHTML={{
                  __html: '<vturb-smartplayer id="vid-68cf097572c453bf1773b5ab" style="display: block; margin: 0 auto; width: 100%; "></vturb-smartplayer>'
                }}
              />
            ) : (
